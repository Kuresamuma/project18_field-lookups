...
def display_webpages(request):
    ...
    QSW=Webpage.objects.filter(topic_name='Badminton').order_by('-name')    
    ...
    QSW=Webpage.objects.filter(name__in=['chinmayii','uma','nikie'])
    QSW=Webpage.objects.filter(Q(topic_name='Cricket') | Q(name='shandhya'))
    QSW=Webpage.objects.all()
...
def display_access(request):
    ...
    QSA=AccessRecords.objects.filter(date__day='13')   
